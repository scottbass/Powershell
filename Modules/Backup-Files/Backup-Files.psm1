...
begin {
   ...
   # Initialize LASTEXITCODE, since it can carry over from previous executions
...
process {
   ...
   if (-not $files) {Write-Warning "There are no files to process."; $global:LASTEXITCODE=99; return $null}
   ...
   if ($temp) {
      ...
      (Get-Date),"Creating archive $Archive`..." | Out-File "$tempLogFile"
      try {
         & "$ArchiveProgram" $ArchiveProgramParms "$Archive" `@"$tempFileList" -w |  Out-File "$tempLogFile" -Append
      }
      catch {
         Write-Error "$_"
      }
      ...
      switch($rc) {
         0 {
            # Use this line to display 7-Zip log with coloring
            Get-Content "$tempLogFile" -ErrorAction SilentlyContinue | Write-Host
            $files
            break
         }
         1 {
            # Use this line to display 7-Zip log with coloring
            Get-Content "$tempLogFile" -ErrorAction SilentlyContinue | Write-Host -ForegroundColor $wfg -BackgroundColor $wbg
            throw "Problems executing 7-Zip, please review log."
            break
         }
         default {
            Get-Content "$tempLogFile" -ErrorAction SilentlyContinue | Write-Host -ForegroundColor $efg -BackgroundColor $ebg
            throw "Problems executing 7-Zip, please review log."
            break
         }
      }
